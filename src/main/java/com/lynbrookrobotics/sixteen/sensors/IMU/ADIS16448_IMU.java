    ...
    import com.lynbrookrobotics.sixteen.sensors.gyro.GyroValue;
    ...
    public class ADIS16448_IMU extends GyroBase implements Gyro, PIDSource, LiveWindowSendable {
      ...
      private GyroValue currentVelocity = new GyroValue(0, 0, 0);
      private GyroValue previousVelocity = new GyroValue(0, 0, 0);
      // last sample time
      ...
      public ADIS16448_IMU() {
        ...
        if (readRegister(kRegPROD_ID) != 16448) {
          m_spi.free();
          m_spi = null;
          DriverStation.reportError("could not find ADIS16448. Instead found " + readRegister(kRegPROD_ID), false);
          return;
        }
      ...
      private void calculate() {
        ...
        while (!m_freed.get()) {
          ...
          synchronized (this) {
            ...
            previousVelocity.set(m_accum_gyro_x, m_accum_gyro_y, m_accum_gyro_z);
            ...
            currentVelocity.set(m_accum_gyro_x, m_accum_gyro_y, m_accum_gyro_z);
      ...
      public GyroValue getRelativeAngle(){
        return ( new GyroValue(
                                  trapaziodalIntegration(previousVelocity.x(), currentVelocity.x()),
                                  trapaziodalIntegration(previousVelocity.y(), currentVelocity.y()),
                                  trapaziodalIntegration(previousVelocity.z(), currentVelocity.z())
                              )
      ...
      private double trapaziodalIntegration(double velocity, double previousVelocity) {
        return (5 * ((velocity + previousVelocity) / 2) / 1000);
      }
    }
